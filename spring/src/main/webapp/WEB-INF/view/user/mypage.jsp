...
<table border ="1">
		...
		<th>
			<c:choose>
				<c:when test="${param.orderCond =='title_asc' }">
				 	 <a href = "mypage.do?orderCond=title_desc">제목 ⬆️ </a>
			    </c:when> 
			    <c:when test="${param.orderCond =='title_desc' }">
					  <a href = "mypage.do?orderCond=title_asc">제목 ⬇️ </a>
				</c:when> 
				<c:otherwise>
		...
		<th>
			<c:choose>
				<c:when test="${param.orderCond =='regdate_asc' }">
				 	 <a href = "mypage.do?orderCond=regdate_desc">작성일 ⬆️ </a>
			    </c:when> 
			    <c:when test="${param.orderCond =='regdate_desc' }">
					  <a href = "mypage.do?orderCond=regdate_asc">작성일 ⬇️ </a>
				</c:when> 
				<c:otherwise>
	...
	<c:forEach var="vo" items="${list}"> <!-- c:forEach 반복태그 -->
		<tr>
			<td>${vo.boardno }</td>
